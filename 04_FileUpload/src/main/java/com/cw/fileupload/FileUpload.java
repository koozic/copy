...
public class FileUpload extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		//1.저장결로(로컬,실제 서버상 경로)
		String savePath="C:\\Users\\soldesk\\Desktop\\teamtitan\\titanbgm";
		String saveRealPath =request.getServletContext().getRealPath("imgFile");
		System.out.println(savePath);
		System.out.println(saveRealPath);
		
		//서버를 나만 쓰는게 아님 여러사람 사용함.
		//로컬 파일 이름은 얼마든지 같거나 중복 될 수 있음.
		//근데 서버에서 겹치면? 중복 되면 안되니까 파일명 처리가 필요함 a1.jpg,a2.jpg,a3.jpg,....
		
		
		//2.파일처리(객체 생성만으로 업로드 기능처리 끝)
		MultipartRequest mr =new MultipartRequest(request, saveRealPath,
				30*1024*1024,
				"utf-8",
				new DefaultFileRenamePolicy());//30mb 파일 최대 허용 용량(넘으면 예외),req.setChara("utf-8");
		//1.값받기(박스 포장을 했으니 그러 까야 req(비닐)에 접근 가능)
		
		
	String name=	mr.getParameter("name");
	System.out.println(name);
	//파일명 읽을때
	//아까 중복 아노디게 처리한것 때문에 올릴떄 파일명이 중요한게 아니고 서버에 올라간'그'이름이 중요함
